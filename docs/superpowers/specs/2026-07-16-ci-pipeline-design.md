...
- **GeoServer is NOT needed in CI** — the suites hit Postgres directly, with one exception: `apps/api/src/geoserver/publish.test.ts` is a live WFS smoke test against a running, published GeoServer. It is env-gated (`describe.skipIf(!process.env.GEOSERVER_URL)`): CI, which sets no `GEOSERVER_URL`, skips it; the local dev stack (`.env`) still runs it. CI omits GeoServer (saves minutes). *(Correction: the first CI run failed on this file; the original claim "never GeoServer" missed it. Fixed by the env-gate.)*
