...
- **Empty conditions.** `applyFilter(features, [])` returns `[]` today, encoding a display decision in the pure core and making "you have not built a filter" indistinguishable from "nothing matched". The pure core drops the special case and returns all features (no predicate = everything matches); `runQuery` refuses an empty filter explicitly, returning `error: 'Chưa có điều kiện lọc'` with no hits, and the panel renders that message where the result list would be. Empty search-bar input is not an error — it simply shows nothing.
